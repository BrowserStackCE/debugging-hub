...
export default function ReplayTool() {
  ...
  const [parseTextLogs, parsingTextLogs, textLogsResult] = usePromise(
    window.browserstackAPI.getAutomateParsedTextLogs
  );
  ...
  const OpenSession = (input: any) => {
    toast
      .promise(
        fetchSessionDetails(input.sessionId).then((res) => parseTextLogs(res)),
        {
  ...
  return (
    <div className="p-5 space-y-4">
      ...
      <Form className="flex gap-4" onFinish={OpenSession}>
        ...
        <button
          disabled={fetchingSession || isExecuting}
          type="submit"
